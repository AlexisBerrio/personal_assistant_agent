...
| 3.5 | `structuredContent` envuelve bajo `{"result": ...}` cuando el retorno anotado es `dict \| None`, pero no para un `dict` simple — inconsistente entre tools, obliga a cada consumidor a conocer el detalle por tool | 🟡 | §A.12 | Hallazgo de 3.2, no arreglado — mismo patrón que el wrapping de arrays (ítem 3.1). Normalizar (siempre envolver, o nunca) antes de que el agente de Fase 4 dependa de leer estas respuestas |
| 3.6 | `delete_task` sin tool MCP (`eliminar_tarea`) ni rama en `_dispatch` — el intent existe en el enum y el prompt lo enseña al LLM, pero ejecutarlo siempre devuelve el fallback genérico de fallo | 🟢 | §A.8 | Hallazgo de 3.1/3.2, no arreglado. `TaskService.delete_task_async` ya existe — falta cablear la tool y la rama de `_dispatch`, sin ambigüedad de diseño |
| 3.7 | Mismatch `task_reference` (lo que el prompt le pide al LLM) vs. `task_id` (lo único que `_dispatch` lee) en `complete_task`/`delete_task` — completar o borrar una tarea por descripción natural ("la del dentista") falla siempre hoy, no es un caso borde | 🟡 | §A.8 | Hallazgo al validar las tools (2026-08-19), no arreglado. Resolver `task_reference` a un `task_id` real exige buscar/matchear contra las tareas del usuario — mismo patrón de "necesita interpretación" del ítem 4.11; decidir si un resolver simple ahora basta o si se difiere al agente (4.3) |
