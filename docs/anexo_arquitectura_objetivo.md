...
| 4.3 | Agente con tools MCP: ejecutor principal de toda acción que el router no pueda despachar con el 100% de lo necesario (criterio de 4.11), no un fallback raro | 🟡 | §A.8 | ✅ Hecho (alcance: resolución de `task_reference`, ver 3.7) — `application/agent/agent.py`: `Agent.resolve_task_reference_to_id` busca en Mongo vía la tool `listar_tareas` (gateada por `Guardrails.evaluate_step`, ítem 4.2) y un LLM dedicado (`OpenAITaskReferenceResolver`, prompt versionado `agent/resolve_task_reference`) decide cuál tarea coincide. `TaskOrchestrator._resolve_task_id` lo invoca cuando `complete_task`/`delete_task` traen `task_reference` sin `task_id`; la escritura final también pasa por guardrails, con `confirmed=True` — decisión documentada en el propio código: el mensaje del usuario ya era una orden explícita, lo único que faltaba era identificar *cuál* tarea, no si proceder. Confirmación interactiva real (round-trip "¿confirmas?") queda fuera de este ítem, depende de 4.10 — documentado como límite consciente, no como deuda oculta. `multi_task` (4.9) y el endpoint conversacional (4.10) también quedan fuera a propósito, tal como se acordó en la intro. 10 tests nuevos (`test_agent.py` + 4 en `test_orchestrator.py`), más verificación real end-to-end (Mongo local + LLM real) contra el caso límite ya discutido: "la tarea odontológica" resuelto correctamente contra "Ir al dentista para limpieza de dientes" |
