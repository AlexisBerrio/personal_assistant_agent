...
| 3.13 | *(hallazgo, Fase 4)* `app.py` (`TaskCreateRequest`/`TaskUpdateRequest`, endpoint CRUD) sigue declarando `dates`/`context_metadata`/`steps` — campos que 3.12 eliminó de `Task` | 🟢 | §A.1 | Sin arreglar. `Task(extra="allow")` los absorbe en silencio (no rompe en runtime), pero `mypy` sí lo detecta (`Unexpected keyword argument` en `app.py`, confirmado vigente el 2026-08-21) porque `app.py` nunca se tocó en el barrido de 3.12 — solo se auditó `task_tools.py`. Corregir: quitar los 3 campos de ambos modelos Pydantic y de la construcción de `Task(...)` en `create_task` |
...
| 4.9 | Router clasifica `multi_task` (2+ acciones en un mensaje) y el agente (4.3) las ejecuta en secuencia vía MCP, en vez de degradar siempre a `clarify` | 🟡 | §A.8 | Diseño sin cambios: `multi_task` es una ruta más de `IntentClassification`. Lo que sí cambió con el rediseño de 4.3: la capacidad de ejecutar **varias tools en secuencia dentro de un mismo turno ya existe y está probada** (el bucle de tool-calling no tiene límite de una sola tool por diseño, ver `test_calls_a_tool_then_returns_the_final_answer` y el caso real con reintento tras error de validación). Lo único que falta es el lado del router: reconocer un mensaje con 2+ acciones distintas y enrutarlo al agente en vez de forzar `clarify` (ítem 2.8) |
...
| 4.14 | *(hallazgo, vía trazas de 4.1)* `_handle_message` extrae hechos de perfil (llamada LLM) antes de clasificar la intención, para todo mensaje sin excepción — incluidos saludos puros que no pueden contener un hecho nuevo | 🟢 | §A.8 | Sin arreglar. Medido con una traza real (`small_talk`, "hola, como estás?"): turno completo de 7.84s, de los cuales 5.36s (68%) son 3 llamadas LLM seriales — `extract_profile_facts` (2.85s) corre primero y no aporta nada a un saludo; `classify_intent` (1.81s) y `answer_small_talk` (0.70s) corren después, cuando la regla rápida de saludo ya sabía la ruta sin tocar el LLM. Dos optimizaciones independientes, ambas verificables con la misma traza: (1) reordenar — clasificar primero (el contexto ya trae los hechos de turnos previos, ver `build_context_summary_async`), extraer perfil solo si la ruta no es `small_talk`; con esto el turno de saludo baja a ~2.9s; (2) `extract_profile_facts` y `classify_intent` son independientes entre sí (mismo input, ningún resultado depende del otro) pero corren en secuencia — con `asyncio.gather` se pagaría el mayor de los dos, no la suma, para las rutas donde ambos sí corren |
...
| 4.16 | Los guardrails de escritura del agente son parcialmente inertes: `steps_used`/`tokens_used` fijos en `0` (sin contador real), y `confirmed=True` forzado sin que el usuario confirme nada de verdad | 🟡 | §A.8 | 🟡 Parcial — (1) resuelto por el rediseño del agente (ver evidencia de 4.3): el bucle de tool-calling acumula `steps_used`/`tokens_used` reales entre pasos, así que el límite de pasos y el presupuesto de tokens ya pueden activarse de verdad (probado en `test_agent.py`: `test_stops_when_the_step_budget_is_exhausted`). (2) Sigue sin arreglar: `confirmed=True` se sigue forzando en cada tool call de escritura dentro del bucle — el agente puede haber elegido el `task_id` equivocado al resolver una referencia, y hoy nada se lo hace confirmar al usuario antes de ejecutar. Cuando exista el endpoint conversacional (4.10) con un round-trip real, las escrituras que decide el agente deben pedir confirmación explícita, no asumirla |
