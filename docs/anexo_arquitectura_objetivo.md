...
| 3.9 | Docstrings insuficientes para un consumidor LLM: no dicen forma de respuesta, comportamiento ante "no encontrado", límite de `listar_tareas` ni valores válidos de `status` | 🟢 | §A.12 | |
| 3.10 | Retornos tipados `dict[str, Any]` → `outputSchema` de FastMCP vacío (verificado contra el protocolo); el docstring es el único contrato hoy | 🟡 | §A.12 | Tipar con modelos reales (Pydantic/TypedDict). Sugerido junto a 4.3, puede adelantarse |
| 3.11 | `listar_tareas` limita a 10 de forma hardcodeada e invisible, sin filtros — reemplaza al 4.12 (mal ubicado en Fase 4, es trabajo de tools) | 🟡 | §A.11 | Diseño: filtros (`estado`, `fecha_desde`, `fecha_hasta`) como parámetros opcionales de la misma tool, no tools separadas; `limite` explícito con techo en servidor |
